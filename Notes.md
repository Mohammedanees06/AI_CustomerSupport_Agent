...
Technical deep-dives, architecture decisions, and concept explanations for contributors and learners.
...
- [Redis](#redis)
- [Voice Architecture — Twilio + ngrok](#voice-architecture--twilio--ngrok)
- [Multi-Tenant Voice Support](#multi-tenant-voice-support)
...
Redis keeps data that is needed right now in fast memory. It is used for caching, shared memory, and scaling apps — so the app does **not** go to the database every time, and gets data instantly.
...
```
User asks: "What are your store timings?"
App → Go to database → Read data → Send reply
This happens every time → slow
```
...
**Features:**
- Needs URL and password
- Used in production
- Accessible from anywhere

### Why We Used Docker

Windows cannot run Redis easily. Docker solves this by running Redis in a small container — no installation headache, works like a real server Redis.

**How Docker Redis works:**

1. Docker starts Redis
2. Redis waits on port 6379
3. Your backend connects to it
4. Redis is ready
...
**User flow:**
```
User → Browser → Backend → Redis → Backend → User
```

---

## Voice Architecture — Twilio + ngrok
...
Twilio gives you:
- A real phone number
- When someone calls, Twilio sends a webhook request to a URL you give it
...
But you don't have a public domain yet during development.
...
### Role of ngrok — Summary

Twilio gives you a real phone number. When someone calls it, Twilio needs a public URL to send the call data to. Your AI is running on localhost (only your laptop), so Twilio can't reach it. ngrok gives your laptop a temporary public link.
...
But Twilio (on the internet) ❌ cannot access localhost — because localhost = only your laptop.

When you run `ngrok http 5000`, ngrok creates a public internet link like:
```
...
So ngrok is just a **bridge between Twilio (internet) and your local AI server**.
...
Twilio sends a POST request to your ngrok URL saying:
...
Your server is running on your laptop (`localhost:5000`). The internet can't reach your laptop directly. So ngrok forwards Twilio's message to your laptop.
...
Twilio contacts your server at `/api/voice/process` with your speech text.
...
## Multi-Tenant Voice Support

### The Problem

The code was originally locked to one specific business ID. If a new company joins the platform, the AI would still answer their phone calls using the first company's data.

### The Solution

Make the code check which phone number was dialed so it can use the right company's data.

1. Add a phone number field to the database schema
2. Save the actual Twilio phone number into the database
3. Update the code so that when a call comes in, it looks up the dialed number, finds the matching company, and uses the correct AI knowledge to reply

### Step-by-Step Flow After a Company Signs Up

**1. The company logs in**

A business owner signs into the platform's frontend dashboard.

**2. They add their phone number**

On their dashboard settings, they enter the Twilio phone number they want to use for their AI voice assistant.

**3. The database saves it**

The backend stores it against their business document:
...
Later, a customer dials `+15551234`. Twilio sends a webhook to your server saying: "Hey, someone is calling +15551234."

**5. The AI makes the match**

The code searches the database for the number `+15551234`. It instantly finds Mary's Bakery, loads her specific AI knowledge, and answers the phone using her data — not any other business's data.
...
In standard Redux, reducers must be entirely synchronous and "pure" — they take the current state and an action, and immediately return a new state. They cannot make API calls, wait for promises, or handle side effects. An async thunk acts as a middleware that bridges this gap, allowing you to fetch data from your backend and update your frontend state accordingly.

Think of an **Async Thunk** as a "middleman" that allows Redux to handle tasks that take time, like fetching data from a website. Normally, Redux is **instant** — you click a button and the state changes immediately. But APIs are **slow**. An Async Thunk sits in between the click and the state change to manage that waiting period.
...
**In short:** It's a way to keep your API logic out of your UI components and give Redux a way to track **loading**, **success**, and **failure** states automatically.

---

### RTK Query

Think of **RTK Query** as the "Auto-Pilot" for data fetching in Redux. Before RTK Query, you had to manually write code to fetch data, track if it was loading, handle errors, and save it to the store. RTK Query does all of that for you automatically.
...
### The Messengers — How We Move Data

These tools are responsible for the actual "phone call" to the server.
...
### The Managers — How We Handle State
...
|---|---|---|
| **Boilerplate** | High — manually create loading, success, and error states in slices | Minimal — automatically generates hooks, reducers, and request state management |
...
| **Best For** | Complex multi-step business logic (sequential API calls or conditional flows) | Standard CRUD operations (GET, POST, PUT, DELETE) |
...
- Async Thunk / RTK Query = **how we manage and store the server response in the app**
...
| Scenario | Use |
|---|---|
| Fetching data from a REST or GraphQL API | RTK Query |
| Uploading a file with a progress bar | RTK Query |
| Complex logic that involves waiting but is not an API | Async Thunk |
| Chaining 5 different actions based on a result | Async Thunk |
...
`routes.jsx` defines:
- What URLs exist
- What component loads for each URL
...
**Authorization:**
- Backend checks JWT
- Backend checks role (owner / staff)
- Backend decides: Can access dashboard? Can create business? Can view analytics?
